...
def author_required(view_func):
    """
    Decorator to ensure only the recipe author can access a view.

    Fetches the recipe and passes it to the view via kwargs['recipe'].
    Returns 403 Forbidden if the user isn't the author.
    """
    @wraps(view_func)
    def wrapper(request, recipe_id, *args, **kwargs):
        recipe = get_object_or_404(Recipe, id=recipe_id)
        if recipe.author != request.user:
            return HttpResponseForbidden('You can only modify your own recipes.')
        kwargs['recipe'] = recipe
        return view_func(request, recipe_id, *args, **kwargs)
    return wrapper
